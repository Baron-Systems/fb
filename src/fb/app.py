...
def create_app(*, db_path: Path, bind_host: str, bind_port: int) -> FastAPI:
    ...
    @app.get("/api/agents/{agent_id}/time")
    def agent_time(agent_id: str) -> JSONResponse:
        """
        Proxy agent time via dashboard to avoid browser CORS issues.
        """
        row = cx.execute("SELECT base_url FROM agents WHERE agent_id=?", (agent_id,)).fetchone()
        if not row:
            return JSONResponse({"ok": False, "error": "not_found"}, status_code=404)

        base_url = str(row["base_url"] or "").strip().rstrip("/")
        if not base_url:
            return JSONResponse({"ok": False, "error": "base_url_missing"}, status_code=400)

        try:
            with http_client() as hc:
                r = hc.get(base_url + "/api/time", timeout=5.0)
                if r.status_code != 200:
                    return JSONResponse(
                        {"ok": False, "error": "agent_error", "status": r.status_code},
                        status_code=502,
                    )
                data = r.json()
                return JSONResponse({"ok": True, "agent_time": data})
        except Exception as e:
    ...
    async def agents_register(request: Request) -> JSONResponse:
        ...
        # Prefer an explicitly advertised base URL from the agent when valid.
        # This is required when request.client.host is rewritten (e.g. proxies/tunnels)
        # and would otherwise become 127.0.0.1.
        advertised_base_url = payload_base_url if payload_base_url.startswith(("http://", "https://")) else ""
        ...
        if token == "reannounce":
            ...
            # Priority:
            # 1) explicitly advertised URL from agent
            # 2) existing saved URL (manual/dashboard value)
            # 3) request-derived fallback
            existing_base_url = str(row["base_url"] or "").strip() if row else ""
            base_url = advertised_base_url or existing_base_url or f"http://{ip}:{agent_port}"
            ...
            if not row:
                cx.execute(
                    ...
                    (
                        ...
                        agent_id,
                        1,
                        json.dumps({"ip": ip, "port": agent_port, "base_url": base_url}),
                    ),
                )
                cx.commit()
        ...
        existing_saved_base_url = str(existing_row["base_url"] or "").strip() if existing_row else ""
        base_url = (
            advertised_base_url
            or existing_saved_base_url
